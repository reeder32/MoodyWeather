...
class LoadingViewController: UIViewController {
    ...
    var connector = OpenWeatherMapApiConnector()
    ...
    func getWeatherForCurrentLocation(location: CLLocation) {
        print(location)
        connector.getWeatherForCurrentLocation(with: location.coordinate.latitude, lon: location.coordinate.longitude) { (weather, error) in
            if error != nil { self.presentPageControl(self.locationIDs); return }
