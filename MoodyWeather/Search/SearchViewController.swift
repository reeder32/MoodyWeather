...
class SearchViewController: UIViewController {
    ...
    lazy var locationButton: UIButton = {
        ...
        b.titleLabel?.font = FontBuilder().setBoldFont(size: 12)
    ...
    var cities: [City] = []
    var results = [City]()
    private let apiConnector = OpenWeatherMapApiConnector()
    ...
    @objc func didTapLocation() {
        ...
        locationManager.didGetLocation = { location, error in
            if let location = location {
                self.locationManager.stop()
                self.apiConnector.getWeatherForCurrentLocation(with: location.coordinate.latitude, lon: location.coordinate.longitude ){ (weather, error) in
...
extension SearchViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.apiConnector.getWeatherForCurrentLocation(with: location?.latitude, lon: location?.longitude) { (weather, error) in
            if let err = error { self.alert(message: err.localizedDescription); return }
